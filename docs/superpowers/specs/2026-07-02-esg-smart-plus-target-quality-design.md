...
`test_enabler_ids_match_snapshot_validator` (JSON == `VALID_ENABLER`) and `test_enablers_present_and_well_formed` (JSON == `EXPECTED_ENABLERS`) enforce the three-way equality.
...
- **Both-non-empty guard (backward-compat, prevents spurious churn).** Report `field: old -> new` ONLY when BOTH old and new are non-empty and differ. A first-time assessment against a pre-v2 baseline (old is `""`/absent → `None`, new is `substantive`) is NOT a reassessment; normalize absent/`None`/`""` as equal so a 29-col snapshot diffed against a 13/19-col baseline shows no spurious "reassessed" rows. (Optionally surface genuine first-time assessments — a target present in both snapshots whose materiality field went blank→value — under a separate **"Newly assessed"** label, never under "reassessed.")
...
- `test_diff.py`:
  ...
  - A **quality-only-change** test: two 29-col snapshots where a target's only change is a materiality field (value/end-year unchanged) — assert the Target-movements block AND the subsection still render (guards the render-guard fix).
  - A **backward-compat no-churn** test: a 19-col `_write` baseline vs a 29-col new snapshot — assert no spurious "Quality reassessed" rows for the empty-old fields.
