...
> 3. Preserve: `topic`, `branch`, `phase`, `status`, `pr`, `plan`, `blockers`, `db_gate`, `review_passes`, `repair`, `repair_enabled`, `advisor_routing`.
...
writes it and nothing rewrites it: nothing that happens during a run changes
either input. A state file written by a pre-change `/ship` has no
...
```
**Advisor routing — read two facts, record always.** Before the state write,
determine and record whether this session's backend is Anthropic's and
...
in the state write below. This is an answer, not a precondition: every
branch records and moves on, and on an Anthropic backend
...
`default_branch`, `spec`, `topic`, `focus_next`, `advisor_routing`).
**Rollback:** if the state-file write fails after the branch was created, run
