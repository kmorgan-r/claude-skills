#!/usr/bin/env python3
"""
Serper.dev port of enrich_recover.py.

Same three recovery paths:
1. Nameless + parseable email local-part -> derive name, search.
2. Nameless + generic inbox -> mark "Generic Contact" score 1.
3. Named but search failed -> LinkedIn-restricted re-query (site:linkedin.com)
   + domain->company anchor mapping for big firms.

Usage:
    python serper_recover.py \
        --input  v16b.csv \
        --output v17.csv \
        --serper-key 9e8c... \
...
def serper_search(api_key: str, query: str, linkedin_only: bool,
...
def enrich_with_params(row: Dict[str, str], first: str, last: str,
    ...
    results = serper_search(api_key, query, linkedin_only=linkedin_only)
    if not results and linkedin_only:
        # Fall back to non-restricted
        results = serper_search(api_key, query, linkedin_only=False)
    if not results:
        ...
        if nameless:
            if is_generic_inbox(email):
                ...
                res = enrich_with_params(row, first2, last2, company, domain,
                                         args.serper_key, linkedin_only=False)
            ...
            res = enrich_with_params(row, first, last, company, domain,
                                     args.serper_key, linkedin_only=True)
            if res["status"] == "ok" and res.get("changed"):
                stats["named_ok"] += 1
                print(f"  -> changed: {', '.join(res['changed'])}", flush=True)
            else:
